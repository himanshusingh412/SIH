/**
 * Deliverable content parsers.
 *
 * Structured providers (the offline generator, and Gemini in JSON mode) emit
 * the schemas below directly. Older projects — and any Gemini reply that came
 * back as prose — are markdown, so every parser degrades to a markdown reader
 * that recovers the same shape. That keeps already-generated deliverables
 * rendering correctly without forcing a regenerate.
 */
...
/** Parses JSON that may be wrapped in ``` fences or padded with prose. */
...
/** Removes markdown emphasis so text reads cleanly inside a slide or card. */
...
/** Splits a markdown pipe-table row into trimmed cells. */
...
/**
 * Splits markdown into sections at the heading level that best represents
 * "one unit of content" — a single H1 is treated as the document title.
 */
function splitSections(markdown: string): { docTitle?: string; sections: Section[] } {
  ...
  if (!candidates.length) {
    // No usable headings — fall back to horizontal rules, then blank-line blocks.
    ...
    return {
      ...
      sections: chunks.map((chunk, i) => {
        ...
        // Only lift the first line out as a heading when there is a body left
        // behind it — otherwise a lone paragraph would produce an empty slide.
        const hasTitle =
  ...
  candidates.forEach((heading, i) => {
    const start = heading.index + 1;
    const end = i + 1 < candidates.length ? candidates[i + 1].index : lines.length;
    sections.push({
...
/**
 * Turns the body of a section into presentable bullet points: real bullets stay
 * bullets, `Label: value` lines and two-column table rows become `Label — value`,
 * and loose paragraphs become one bullet each.
 */
function linesToBullets(lines: string[]): { bullets: string[]; notes: string[] } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^\s*>\s?/.test(line) || /^\**\s*(speaker notes?|narration)\s*\**\s*:/i.test(trimmed)) {
      flushParagraph();
      const note = stripInline(trimmed.replace(/^\s*>\s?/, '').replace(/^\**\s*(speaker notes?|narration)\s*\**\s*:/i, ''));
    ...
    if (isTableRow(line)) {
      ...
      const isHeaderRow = /^(metric|item|field|key|name|category)$/i.test(cells[0] || '');
      if (isHeaderRow) continue;
      bullets.push(cells.length >= 2 ? `${cells[0]} — ${cells.slice(1).join(' · ')}` : cells[0]);
      continue;
    }

    if (isBullet(line)) {
      flushParagraph();
      const text = stripInline(stripBullet(line));
      if (text) bullets.push(text);
      continue;
    }
  ...
  flushParagraph();
  return { bullets: bullets.filter(Boolean), notes };
}

// ------------------------------------------------------------------- slides

function normalizeSlide(raw: any, index: number): Slide {
  const bullets = raw?.bulletPoints || raw?.bullets || raw?.points || raw?.content || [];
  return {
    slideNumber: Number(raw?.slideNumber) || index + 1,
    title: stripInline(raw?.title || raw?.heading || `Slide ${index + 1}`),
    bulletPoints: (Array.isArray(bullets) ? bullets : [bullets])
...
/** Reads a PRESENTATION deliverable as a slide deck, from JSON or markdown. */
export function parseSlides(content: string | null | undefined): Slide[] {
  ...
  sections.forEach((section) => {
    ...
    // Long sections spill onto continuation slides rather than overflowing.
  ...
  // Prepend a title slide when the document had a single top-level heading.
  if (docTitle) {
    slides.unshift({
      slideNumber: 1,
      title: docTitle,
...
const METRIC_LABEL_MAX = 46;
const METRIC_VALUE_MAX = 34;

/** Reads an INFOGRAPHIC deliverable as headline metrics plus callout panels. */
export function parseInfographic(content: string | null | undefined): InfographicLayout | null {
  ...
  const pushMetric = (label: string, value: string) => {
    ...
    if (cleanLabel.length > METRIC_LABEL_MAX || cleanValue.length > METRIC_VALUE_MAX) return;
  ...
  sections.forEach((section) => {
    ...
    section.lines.forEach((line) => {
      ...
      if (labelled) {
        ...
        if (stripInline(labelled[2]).length > METRIC_VALUE_MAX) {
...
/**
 * Reads a VIDEO_PACKAGE deliverable as a storyboard. Returns null when the
 * content has no recognisable scenes so the caller can fall back to prose.
 */
